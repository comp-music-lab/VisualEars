...
\begin{figure}
\centering
\includegraphics{Power_analysis_TELESCOPE_files/figure-latex/unnamed-chunk-7-1.pdf}
...
\begin{figure}
\centering
\includegraphics{Power_analysis_TELESCOPE_files/figure-latex/pilot-rep-1.pdf}
...
\NormalTok{dat.den.gr }\OtherTok{\textless{}{-}} \FunctionTok{clmm\_generate\_data}\NormalTok{(}\AttributeTok{n\_participants =} \DecValTok{10000}\NormalTok{,}
                                 \AttributeTok{n\_trials =} \DecValTok{3}\NormalTok{,}
                                 \AttributeTok{control\_distribution =} \FunctionTok{c}\NormalTok{(.}\DecValTok{04}\NormalTok{, .}\DecValTok{11}\NormalTok{, .}\DecValTok{59}\NormalTok{, .}\DecValTok{22}\NormalTok{, .}\DecValTok{04}\NormalTok{),}
                                 \AttributeTok{effect =} \FloatTok{0.4}\NormalTok{,}
                                 \AttributeTok{participant\_variation =} \DecValTok{1}\NormalTok{,}
                                 \AttributeTok{within\_subject =} \ConstantTok{TRUE}\NormalTok{,}
                                 \AttributeTok{control\_weight =}\NormalTok{ .}\DecValTok{5}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{group =} \FunctionTok{ifelse}\NormalTok{(group }\SpecialCharTok{==} \DecValTok{0}\NormalTok{, }\StringTok{"Low"}\NormalTok{, }\StringTok{"High"}\NormalTok{)) }\SpecialCharTok{|\textgreater{}}
...
\begin{figure}
\centering
\includegraphics{Power_analysis_TELESCOPE_files/figure-latex/final-sim-1.pdf}
...
\begin{figure}
\centering
\includegraphics{Power_analysis_TELESCOPE_files/figure-latex/unnamed-chunk-12-1.pdf}
...
\NormalTok{sample\_size.clmm }\OtherTok{=} \DecValTok{80} 
...
For each random sample, we adjusted the number of participants per country (in this case, \emph{n} = 80) to achieve the target statistical power of at least \(1 - \beta = 0.95\) for both the Density and Arousal models, for the contrast between Tempo levels (i.e., averaged over the levels of instrumentation, participant country and music country).
...
\CommentTok{\# Loop to fit a model for each random sample and extract the p{-}value of the Tempo contrast}
\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\SpecialCharTok{:}\NormalTok{num\_sims.clmm)\{}
\NormalTok{  samp }\OtherTok{\textless{}{-}}\NormalTok{ samples.clmm.long }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{filter}\NormalTok{(sample.clmm }\SpecialCharTok{==}\NormalTok{ i)}
...
\begin{figure}
\centering
\includegraphics{Power_analysis_TELESCOPE_files/figure-latex/unnamed-chunk-15-1.pdf}
...
\begin{figure}
\centering
\includegraphics{Power_analysis_TELESCOPE_files/figure-latex/unnamed-chunk-17-1.pdf}
...
\begin{figure}
\centering
\includegraphics{Power_analysis_TELESCOPE_files/figure-latex/plot-mod-ex-1.pdf}
...
\begin{figure}
\centering
\includegraphics{Power_analysis_TELESCOPE_files/figure-latex/unnamed-chunk-18-1.pdf}
...
\begin{figure}
\centering
\includegraphics{Power_analysis_TELESCOPE_files/figure-latex/unnamed-chunk-19-1.pdf}
...
\textbf{R version 4.3.3 (2024-02-29 ucrt)}
...
\emph{pander(v.0.6.5)}, \emph{beepr(v.1.3)}, \emph{scales(v.1.3.0)}, \emph{performance(v.0.10.9)}, \emph{kableExtra(v.1.4.0)}, \emph{ordinal(v.2023.12-4)}, \emph{emmeans(v.1.10.0)}, \emph{ggpubr(v.0.6.0)}, \emph{lubridate(v.1.9.3)}, \emph{forcats(v.1.0.0)}, \emph{stringr(v.1.5.1)}, \emph{dplyr(v.1.1.4)}, \emph{purrr(v.1.0.2)}, \emph{readr(v.2.1.5)}, \emph{tidyr(v.1.3.1)}, \emph{tibble(v.3.2.1)}, \emph{ggplot2(v.3.5.0)}, \emph{tidyverse(v.2.0.0)}, \emph{knitr(v.1.45)} and \emph{tictoc(v.1.2)}

\textbf{loaded via a namespace (and not attached):}
\emph{tidyselect(v.1.2.0)}, \emph{viridisLite(v.0.4.2)}, \emph{farver(v.2.1.1)}, \emph{fastmap(v.1.1.1)}, \emph{TH.data(v.1.1-2)}, \emph{digest(v.0.6.34)}, \emph{estimability(v.1.5)}, \emph{timechange(v.0.3.0)}, \emph{lifecycle(v.1.0.4)}, \emph{survival(v.3.5-8)}, \emph{magrittr(v.2.0.3)}, \emph{compiler(v.4.3.3)}, \emph{rlang(v.1.1.3)}, \emph{tools(v.4.3.3)}, \emph{utf8(v.1.2.4)}, \emph{yaml(v.2.3.8)}, \emph{ggsignif(v.0.6.4)}, \emph{labeling(v.0.4.3)}, \emph{xml2(v.1.3.6)}, \emph{multcomp(v.1.4-25)}, \emph{abind(v.1.4-5)}, \emph{rsconnect(v.1.2.1)}, \emph{withr(v.3.0.0)}, \emph{numDeriv(v.2016.8-1.1)}, \emph{grid(v.4.3.3)}, \emph{fansi(v.1.0.6)}, \emph{xtable(v.1.8-4)}, \emph{colorspace(v.2.1-0)}, \emph{MASS(v.7.3-60.0.1)}, \emph{tinytex(v.0.49)}, \emph{insight(v.0.19.8)}, \emph{cli(v.3.6.2)}, \emph{mvtnorm(v.1.2-4)}, \emph{rmarkdown(v.2.25)}, \emph{generics(v.0.1.3)}, \emph{rstudioapi(v.0.15.0)}, \emph{tzdb(v.0.4.0)}, \emph{audio(v.0.1-11)}, \emph{splines(v.4.3.3)}, \emph{vctrs(v.0.6.5)}, \emph{Matrix(v.1.6-5)}, \emph{sandwich(v.3.1-0)}, \emph{carData(v.3.0-5)}, \emph{bookdown(v.0.37)}, \emph{car(v.3.1-2)}, \emph{hms(v.1.1.3)}, \emph{rstatix(v.0.7.2)}, \emph{systemfonts(v.1.0.5)}, \emph{glue(v.1.7.0)}, \emph{codetools(v.0.2-19)}, \emph{cowplot(v.1.1.3)}, \emph{stringi(v.1.8.3)}, \emph{gtable(v.0.3.4)}, \emph{munsell(v.0.5.0)}, \emph{pillar(v.1.9.0)}, \emph{htmltools(v.0.5.7)}, \emph{R6(v.2.5.1)}, \emph{ucminf(v.1.2.1)}, \emph{evaluate(v.0.23)}, \emph{lattice(v.0.22-5)}, \emph{highr(v.0.10)}, \emph{backports(v.1.4.1)}, \emph{broom(v.1.0.5)}, \emph{Rcpp(v.1.0.12)}, \emph{svglite(v.2.1.3)}, \emph{coda(v.0.19-4.1)}, \emph{gridExtra(v.2.3)}, \emph{nlme(v.3.1-164)}, \emph{xfun(v.0.42)}, \emph{zoo(v.1.8-12)} and \emph{pkgconfig(v.2.0.3)}
